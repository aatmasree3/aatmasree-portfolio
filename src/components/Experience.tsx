    ...
    <div className="grid grid-cols-1 gap-8">
      {experiences.map((exp) => (
        <div key={exp.company} className="bg-card rounded-xl border border-border shadow-lg p-6">
          <h3 className="text-xl font-bold text-secondary mb-1 font-heading">{exp.position} <span className="font-normal text-foreground">@ {exp.company}</span></h3>
          <div className="text-sm text-muted-foreground mb-2 font-sans">
            {exp.location} &middot; <span className=" italic">{exp.date}</span>
          </div>
          <ul className="list-disc pl-4 text-foreground font-sans">
            {exp.desc.map((d, i) => (
              <li key={i}>{d}</li>
            ))}
          </ul>
